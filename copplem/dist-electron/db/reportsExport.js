...
exports.exportReportsPdf = exportReportsPdf;
exports.warmupReportsExports = warmupReportsExports;
...
function resolveOutDir(p) {
    const base = (p.outDir && p.outDir.trim()) ? p.outDir.trim() : electron_1.app.getPath("desktop");
    if (!p.createSubfolder)
        return base;
    const dir = node_path_1.default.join(base, `Reportes_${p.slug}`);
    try {
...
function uniquePath(dir, fname) {
    const ext = node_path_1.default.extname(fname);
    const base = node_path_1.default.basename(fname, ext);
    let candidate = node_path_1.default.join(dir, fname);
    let i = 2;
    while (node_fs_1.default.existsSync(candidate)) {
        candidate = node_path_1.default.join(dir, `${base} (${i})${ext}`);
        i++;
    }
    return candidate;
}
...
function getReportsStandalone(slug, p) {
    ...
    `).get({ from: prevFromISO, to: prevToISO });
        ...
        const pct = (nowV, prevV) => (prevV > 0 ? ((nowV - prevV) / prevV) * 100 : 0);
        // Tendencia
        const trend = [];
...
function loadBrandLogoSvgDataUrl() {
    const devPath = node_path_1.default.join(process.cwd(), "src", "assets", "logo.svg");
    const prodPath = node_path_1.default.join(process.resourcesPath || process.cwd(), "assets", "logo.svg");
    const p = node_fs_1.default.existsSync(devPath) ? devPath : (node_fs_1.default.existsSync(prodPath) ? prodPath : null);
    if (!p)
        return null;
    try {
        const svg = node_fs_1.default.readFileSync(p, "utf8");
...
function svgBars(categories, money) {
    const w = 680, h = 260, pad = 36;
...
function svgBarsYears(points, money) {
    const w = 680, h = 260, pad = 36;
    const max = Math.max(1, ...points.map(p => p.total));
...
function formatDeltaSmart(now, prev) {
    if (prev === 0 && now === 0)
        return "0.0%";
    if (prev === 0 && now > 0)
        return "↑ nuevo";
...
/* ========= PDF ========= */
async function exportReportsPdf(slug, p) {
    const mode = p?.allTime ? "total" :
        p?.group === "day" ? "week" :
    ...
    const prev = (() => {
      ...
      FROM ventas
      WHERE date(fecha,'localtime') BETWEEN date(@from) AND date(@to)
    `).get({ from: data.period.prevFrom, to: data.period.prevTo });
        const viPrev = db.prepare(`
      SELECT COALESCE(SUM(vi.subtotal),0) AS revenue_items,
             COALESCE(SUM(COALESCE(vi.costo_unit_ref,0) * vi.cantidad),0) AS cost_items,
             COALESCE(SUM(vi.cantidad),0) AS units
    ...
    if (brandSvg) {
        ...
        const png = (0, paths_1.logoPngPathForSlug)(slug);
        if (node_fs_1.default.existsSync(png)) {
            const buf = node_fs_1.default.readFileSync(png);
    ...
    const subtitle = data.mode === "week" ? "Semanales · últimos 7 días" :
...
<body>
  <div class="page">
    ...
    try {
        ...
        node_fs_1.default.writeFileSync(absPath, pdf);
    }
    finally {
        try {
            win.destroy();
...
let ExcelWarmupPromise = null;
function warmupReportsExports() {
    if (!ExcelWarmupPromise) {
        ExcelWarmupPromise = Promise.resolve().then(() => __importStar(require("exceljs")));
    }
}
/* ====================== EXPORTAR A EXCEL ======================= */
async function exportReportsXlsx(slug, p) {
    const mode = p?.allTime ? "total" :
        p?.group === "day" ? "week" :
            p?.group === "month" ? "month" : "year";
